...
class STEM extends React.Component {
  ...
  async componentDidMount() {
    try {
      const res = await axios.get(`https://newsapi.org/v2/top-headlines?category=science&country=za&apiKey=${newsApiKey}`)
  ...
  changeCountry = (e) => {
    ...
    this.getArticles(e.target.value, category)
  }

  changeCategory = (category) => {
    const { countryCode } = this.state
    this.setState({ category })
    this.getArticles(countryCode, category)
  }
  ...
  render() {
    const { articles, countries, category, countryCode, modalActive, currentArticle } = this.state
