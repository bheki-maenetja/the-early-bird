...
class Business extends React.Component {
  ...
  async componentDidMount() {
    try {
      const res = await axios.get(`https://newsapi.org/v2/top-headlines?country=za&category=business&apiKey=${newsApiKey}`)
  ...
  getArticles = async (code) => {
    try {
      const res = await axios.get(`https://newsapi.org/v2/top-headlines?country=${code}&category=business&apiKey=${newsApiKey}`)
      this.setState({ articles: res.data.articles })
    } catch (err) {
      console.log(err)
    }
    ...
    return (
      ...
      <section className="section" style={{ flexGrow: '1', overflowY: 'scroll' }}>
        <div className="container">
          <h1 className="title is-1 has-text-centered">Top Business Headlines from around the World</h1>
          <hr />
